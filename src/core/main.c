...
static void write_root_script(void) {
  ...
  int n = snprintf(
      ...
      "  exit 1\n"
       "fi\n"
       "# shizuku mode: hand root to Shizuku privileged api, skip the KernelSU chain below\n"
       "if [ -n \"$GHOSTLOCK_SHIZUKU_STARTER\" ]; then\n"
       "  echo \"[*] shizuku starter=$GHOSTLOCK_SHIZUKU_STARTER\" >>\"$LOG\"\n"
       "  if [ -n \"$GHOSTLOCK_SHIZUKU_APK\" ]; then\n"
       "    echo \"[*] shizuku manager apk=$GHOSTLOCK_SHIZUKU_APK\" >>\"$LOG\"\n"
       "  fi\n"
       ...
       "  fi\n"
       "  if [ -n \"$GHOSTLOCK_SHIZUKU_APK\" ]; then\n"
       "    \"$GHOSTLOCK_SHIZUKU_STARTER\" \"--apk=$GHOSTLOCK_SHIZUKU_APK\" >>\"$LOG\" 2>&1\n"
       "  else\n"
